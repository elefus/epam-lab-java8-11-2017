...
public class Exercise1 {
    ...
    public void findPersonsBeganCareerInEpam() {
        ...
        List<Person> startedFromEpam = employees.stream()
                .filter(employee -> employee.getJobHistory().stream().map(JobHistoryEntry::getEmployer).findFirst().get().equals("EPAM"))
                .map(Employee::getPerson).collect(Collectors.toList());
    ...
    public void findAllCompanies() {
        ...
        Set<String> companies = employees.stream().map(Employee::getJobHistory)
                .flatMap(jobHistoryEntries -> jobHistoryEntries.stream().map(JobHistoryEntry::getEmployer))
