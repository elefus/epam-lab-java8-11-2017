...
public class Exercise1 {
    ...
    public void findPersonsEverWorkedInEpam() {
        ...
        List<Person> personsEverWorkedInEpam = employees.stream()
                .filter(employee -> employee.getJobHistory().stream().anyMatch(e -> e.getEmployer().equals("EPAM")))
    ...
    public void findMinimalAgeOfEmployees() {
        ...
        Integer minimalAge = employees.stream().map(Employee::getPerson).min(Person::compareTo).get().getAge();
