...
public class Exercise1 {
    ...
    private static BiFunction <Person, Person, Integer> createExtractorAgeOfPersonWithTheLongestFullName(Function <Person, String> propertyExtractor)  {
        return (p1, p2) -> {
            if (propertyExtractor.apply(p1).length() > propertyExtractor.apply(p2).length()) {
                return p1.getAge();
            }
            return p2.getAge();
        };
